...
export async function PATCH(req: Request, { params }: Params) {
    try {
        ...
        if (!Number.isInteger(couponId)) {
            const apiError = new ApiError(400, "Invalid coupon id");
            return NextResponse.json(apiError, { status: apiError.statusCode });
        }
        ...
        const apiResponse = new ApiResponse(200, coupon, "Coupon updated successfully");
...
export async function DELETE(_req: Request, { params }: Params) {
    try {
        ...
        const apiResponse = new ApiResponse(200, null, "Coupon deleted successfully");
        return NextResponse.json(apiResponse, { status: apiResponse.statusCode });
    } catch (error) {
